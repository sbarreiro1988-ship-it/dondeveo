...
# Configurar via variable de entorno GROQ_API_KEY en el servidor
# o editar directamente este archivo en el servidor (no commitear la key)
    ...
    for feed_url in RSS_FEEDS:
        ...
        try:
            ...
            for entry in feed.entries[:6]:
                ...
                # Obtener thumbnail — RSS media tags primero, OG image como fallback
                thumbnail = None
                ...
                # Si todavía no hay imagen, extraer og:image del artículo original
                if not thumbnail and url:
                    try:
                        og_resp = requests.get(url, timeout=6,
                            headers={"User-Agent": "Mozilla/5.0 (compatible; DondeVeoBot/1.0)"},
                            allow_redirects=True)
                        if og_resp.ok:
                            import re as _re
                            og_match = (_re.search(r'property="og:image"\s+content="([^"]+)"', og_resp.text)
                                        or _re.search(r'content="([^"]+)"\s+property="og:image"', og_resp.text)
                                        or _re.search(r'name="twitter:image"\s+content="([^"]+)"', og_resp.text))
                            if og_match:
                                candidate = og_match.group(1).strip()
                                if candidate.startswith("http") and not ".svg" in candidate:
                                    thumbnail = candidate
                    except Exception:
                        pass

                log(f"   ✍  Reescribiendo: {title_raw[:60]} {'📷' if thumbnail else '🚫foto'}")
