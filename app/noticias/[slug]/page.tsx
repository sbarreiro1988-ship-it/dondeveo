...
async function fetchArticle(slug: string): Promise<ArticleData | null> {
  // 1️⃣ Primero: leer desde filesystem (cPanel — sin problemas de DNS)
  const filePath = process.env.NEWS_FILE_PATH;
  if (filePath) {
    try {
      const fs = await import('fs');
      const raw = fs.readFileSync(`${filePath}/articles/${slug}.json`, 'utf8');
      return JSON.parse(raw) as ArticleData;
    } catch { /* seguir */ }
  }

  // 2️⃣ Segundo: artículos del script via HTTP (Vercel / build time)
  ...
  // 3️⃣ Fallback: artículos estáticos propios de DondeVeo
